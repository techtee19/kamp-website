  ...
  return (
    <div className="absolute inset-0 -z-20" aria-hidden="true">
      {heroImages.map((image, index) => (
        <Image key={image} src={image} alt="" fill priority={index === 0} sizes="100vw" className={`object-contain transition-opacity duration-[1200ms] ease-in-out ${index === activeImage ? 'opacity-100' : 'opacity-0'}`} />
      ))}
    </div>
  )
}
